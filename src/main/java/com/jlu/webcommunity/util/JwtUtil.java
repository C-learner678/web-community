...
public class JwtUtil {
    ...
    public String createSession(User user){
        ...
        String token = JWT.create().withIssuer(jwtProperties.getIssuer())
                ...
                .sign(algorithm);
        redisUtil.set(RedisConstant.userSessionKey + token, String.valueOf(user.getId()), jwtProperties.getExpire());
        return token;
    }

    //用户退出登录，删除会话
    public void removeSession(String token){
        redisUtil.del(RedisConstant.userSessionKey + token);
    }
    ...
    public User getUserBySession(String token){
        ...
        String userId2 = (String) redisUtil.get(RedisConstant.userSessionKey + token);
        if(userId2 == null){
            return null;
        }
